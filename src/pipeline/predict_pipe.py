...
class pred_pipeline:
    ...
    def predict(self,features):
        try:
            ...
            propabilities=model.predict_proba(data_trans)
            return output,propabilities[0][1]
        except Exception as e:
            raise CustomException(e,sys)
class input_data:
    
    def __init__(self, Age: int, Income: int, Home: str, Emp_length: float, Intent: str, Amount: int, Rate: float, Status: int, Percent_income: float, Cred_length: int):
        self.Age = Age
        self.Income = Income
        self.Home = Home
